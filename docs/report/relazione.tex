...
\begin{itemize}
    \item \textbf{Normalizzazione e Gestione dei Ritardi:} Tramite script dedicati nel processore \texttt{UpdateRecord}, la pipeline neutralizza eventuali valori di ritardo negativi errati e gestisce l'imputazione dei valori nulli, secondo la logica architetturale e di business.
...
Nell'implementazione RDD, si è evitato l'uso dell'operatore \texttt{groupByKey}, un'operazione che può causare gravi colli di bottiglia prestazionali a causa del trasferimento di dati sulla rete durante la fase di \textit{shuffle}.
...
\begin{itemize}
    \item \textbf{Analisi dell'Intensità:} Ignorando i valori nulli, il calcolo della media includerebbe solo i voli con ritardo conclamato. Sebbene utile per misurare la severità di un problema quando si verifica, questo approccio distorcerebbe la visione d'insieme, sovrastimando l'impatto delle singole cause sull'operatività totale del vettore.
    \item \textbf{Analisi del Contributo Medio (Scelta SAE):} Trattando i valori mancanti come zero, SAE assicura che il denominatore della media sia costituito dalla totalità dei voli non cancellati e non deviati della compagnia. In questo modo, il risultato esprime quanto mediamente ogni causa (es. condizioni meteo o problemi tecnici) pesi sull'intera flotta del vettore.
\end{itemize}
...
L'integrità di questa logica è verificata in fase di esecuzione distribuita assicurando che tutti i record che superano il filtro dei 500 voli contribuiscano in modo uniforme al calcolo della classifica.
