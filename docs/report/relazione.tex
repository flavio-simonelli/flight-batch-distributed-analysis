...
Nelle sezioni successive, esploreremo in dettaglio come la modularità implementata a livello di codice sorgente, attraverso l'utilizzo di variabili nei vari framework e l'uso estensivo del pattern GoF Factory e interfacce astratte in Java per Spark, si rifletta coerentemente nella modularità dell'infrastruttura di deployment.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.47\textwidth]{../graphs/BigDataStack}
...
Questa netta separazione assicura che il sistema sia "future-proof": è possibile scalare o sostituire il motore di processing o qualsiasi componente in generale senza impattare gli altri livelli\@.
...
L'orchestrazione dei flussi di lavoro in Sky Analytics Engine è demandata a una suite di tre DAG.
...
\begin{itemize}
    \item \item \textbf{Normalizzazione e Gestione dei Ritardi:} Tramite script dedicati nel processore \texttt{UpdateRecord}, la pipeline neutralizza i ritardi negativi errati e gestisce i valori mancanti delle singole cause. Nello specifico, i valori vengono mantenuti a \texttt{null} se il ritardo all'arrivo (\texttt{arr\_delay}) è inferiore a 15 minuti (come imposto per legge); in caso contrario, se è presente almeno una causa di ritardo diversa da zero, i restanti valori nulli vengono sovrascritti a 0.
...
Il cuore della persistenza intermedia di Sky Analytics Engine è rappresentato dal Data Lake, implementato tramite Hadoop HDFS o MinIO in ambienti on-premise e Amazon S3 in configurazioni cloud.
Questo strato funge da \textit{Single Source of Truth}, sfruttando la capacità intrinseca del Data Lake di accogliere e centralizzare dati eterogenei (strutturati, semi-strutturati e grezzi) senza la necessità di un pre-processing o di uno schema rigido preventivo (\textit{schema-on-read}).

L'infrastruttura ospita sia i dataset grezzi originari acquisiti da NiFi, sia i dati successivamente raffinati e convertiti in formato Parquet. Nelle distribuzioni cloud basate su Amazon S3, questa architettura abilita un disaccoppiamento totale tra le risorse di storage e quelle di calcolo, garantendo una scalabilità indipendente. Inoltre, la strutturazione dei dati raffinati in formato colonnare (Parquet) permette a Spark di eseguire ottimizzazioni avanzate, come il \textit{predicate pushdown} e la \textit{column projection}, riducendo drasticamente l'I/O complessivo durante le fasi di analisi.
...
L'architettura garantisce l'integrità del calcolo attraverso una suite di test di coerenza locale (\texttt{QueryConsistencyTest.java}).
Questi test caricano il dataset Parquet reale fornito e verificano che i risultati ottenuti tramite le tre diverse astrazioni di Spark (RDD, DataFrame e SQL) siano matematicamente equivalenti entro tolleranze definite (necessarie per gli algoritmi di sketching), assicurando che la logica di business sia preservata indipendentemente dal backend scelto\@.
...
La \textit{Query 1} richiede il calcolo di statistiche mensili (media, min, max, tasso di cancellazione) per specifiche compagnie. La struttura logica dell'esecuzione è visualizzabile nel DAG di Spark riportato in Appendice~\ref{sec:appendix_spark_dags} (Fig.~\ref{fig:dag_q1}).

L'implementazione basata su RDD si articola nelle seguenti fasi:
\begin{itemize}
    \item \textbf{Map-Side Aggregation con \texttt{aggregateByKey}:} Si è evitato l'uso dell'operatore \texttt{groupByKey}, un'operazione che può causare colli di bottiglia prestazionali a causa del trasferimento di dati sulla rete durante la fase di \textit{shuffle}. Al suo posto è stato utilizzato \texttt{aggregateByKey}, il quale combina i valori parzialmente all'interno di ogni singola partizione del nodo worker prima di trasferirli sulla rete, riducendo drasticamente il volume di dati movimentato nello shuffle.
    ...
    \]
    dove $D_{dep}$ è il ritardo, $N_{vld}$ i voli validi e $N_{tot}$ i totali.
...
Questa condizione genera una significativa presenza di valori mancanti (\textit{null}) nel dataset.

Al fine di rispondere correttamente al requisito di calcolare il \textbf{contributo medio} delle diverse cause di ritardo, SAE implementa una strategia di imputazione a zero per i valori mancanti (utilizzando l'operatore \texttt{coalesce(col, 0)} in Spark e pre-elaborando i record parziali in NiFi).
...
\begin{itemize}
    ...
    Sebbene utile per misurare la severità di un problema quando si verifica, questo approccio distorcerebbe la visione d'insieme, sovrastimando l'impatto delle singole cause sull'operatività totale del vettore.
    \item \textbf{Analisi del Contributo Medio :} Trattando i valori mancanti come zero, SAE assicura che il denominatore della media sia costituito dalla totalità dei voli non cancellati e non deviati della compagnia.
    In questo modo, il risultato esprime quanto mediamente ogni causa pesi sull'intera flotta del vettore.
\end{itemize}

Tale approccio fornisce una visione realistica della resilienza del business, permettendo di identificare quali compagnie riescano effettivamente a mitigare l'impatto di specifiche cause su scala globale, anziché focalizzarsi esclusivamente sui casi limite. Il piano di esecuzione distribuito è composto da tre job sequenziali (Fig.~\ref{fig:dag_q2} in Appendice~\ref{sec:appendix_spark_dags}).
...
Attraverso l'integrazione della libreria Apache DataSketches, è stato implementato lo stimatore Karnin-Lang-Liberty (KLL). Parallelamente, per offrire una maggiore flessibilità analitica, il sistema integra anche l'algoritmo T-Digest. Entrambi gli stimatori sono configurabili a runtime tramite la proprietà \texttt{percentileAlgorithm} in \texttt{ApplicationConfig.java}.
...
Questo permette di calcolare i percentili orari in un singolo passaggio sui dati, con una complessità spaziale fissa indipendente dalla dimensione del dataset. La complessità del piano fisico è riflessa nei due job Spark visibili in Fig.~\ref{fig:dag_q3} (Appendice~\ref{sec:appendix_spark_dags}).

Nell'implementazione specifica RDD, la query si divide in due pipeline parallele:
\begin{itemize}
    \item \textbf{Pipeline dei Percentili (Sketching):} Sfrutta l'operatore \texttt{combineByKey} per gestire l'istanziazione, l'aggiornamento e l'unione dei buffer binari che rappresentano lo stato interno dello sketch scelto (KLL o T-Digest). La struttura dati scambiata durante la \textit{combine} è:
    \[
    ...
    \item \textbf{Pipeline Globale (Min/Max):} Opera parallelamente per calcolare i valori estremi per ogni \texttt{Carrier}. Utilizza \texttt{aggregateByKey} con un accumulatore di dimensione fissa:
    \[
    \mathcal{A}_{Q3} = \langle \min(D_{dep}),\; \max(D_{dep}) \rangle
    \]
\end{itemize}
Entrambe le pipeline agiscono su un RDD preventivamente filtrato e cachato in memoria (\texttt{cache()}) per evitare la rilettura ripetuta dei medesimi record dal livello di storage.
...
A differenza delle analisi precedenti, questa query è implementata unicamente tramite le API DataFrame/SQL. Il framework di machine learning di Spark (\texttt{MLlib}), e in particolare le sue funzionalità di \textit{feature extraction} (come \texttt{StandardScaler} e \texttt{PCA}) e le implementazioni ottimizzate del K-Means, dipendono strutturalmente dall'astrazione \texttt{Dataset<Row>} e dai tipi \texttt{Vector} dell'API \texttt{ml.linalg}. Riscrivere il processo utilizzando le API RDD sottostanti annullerebbe i benefici di tali ottimizzazioni ed esporrebbe ad operazioni scalari inefficienti.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.50\textwidth]{../graphs/DeploymentView}
...
\begin{itemize}
    ...
    Ciascun nodo ospita un demone worker di Spark e un datanode HDFS all'interno di container Docker coordinati tramite Docker Compose.
...
% Placed here to avoid breaking the flow of the text, as the figure is quite large and would disrupt the reading if placed in the middle of the discussion.
\begin{figure*}[!t]
    \centering
    \begin{tabular}{cc}
...
Un aspetto metodologico di rilievo è offerto dal confronto tra i tre approcci di calcolo  osservabile proprio sulla distribuzione del 50° percentile.
I grafici (riportati in Appendice~\ref{sec:appendix_results}) mostrano che i risultati ottenuti tramite l'approssimazione KLL coincidono quasi sistematicamente con quelli derivanti dall'algoritmo GK integrato nelle API strutturate.
Questa convergenza dei risultati valida sperimentalmente l'accuratezza degli algoritmi di sketching scelti, confermando che SAE è in grado di fornire insight affidabili anche attraverso tecniche di calcolo probabilistico che abbattono l'overhead computazionale rispetto al calcolo esatto.
...
L'applicazione dell'algoritmo K-Means con selezione automatica del modello tramite Silhouette Score ha identificato $K=3$ come il numero ottimale di raggruppamenti statistici per descrivere i vettori (Fig.~\ref{fig:results_q4_cluster_distribution}).
La proiezione spaziale tramite \textbf{Principal Component Analysis} (Fig.~\ref{fig:results_q4_cluster_distribution}) e la matrice di classificazione delineano tre cluster operativi ben definiti (i valori medi delle feature per ciascun cluster sono riportati in Fig.~\ref{fig:results_q4_cluster_center} e sono stati normalizzati per evidenziare le differenze relative tra i cluster, mentre la Tabella \ref{tab:results_q4_metrics} riporta i corrispondenti valori operativi reali per ogni vettore):


\begin{itemize}
    \item \textbf{Cluster 0 - Inefficienza Operativa e Colli di Bottiglia:} Comprende \texttt{AA}, \texttt{B6}, \texttt{OO}, \texttt{F9}, \texttt{OH}.
    Questi vettori mostrano ritardi medi elevati e una marcata instabilità operativa. PSA Airlines (\texttt{OH}) e Frontier (\texttt{F9}) guidano questo gruppo con ritardi all'arrivo rispettivamente di $17,16$ e $11,32$ minuti. Il cluster è caratterizzato da un forte effetto di propagazione (\textit{late aircraft delay} elevato) e, nel caso di \texttt{OH}, da un tasso di cancellazione critico del $6,09\%$.
    \item \textbf{Cluster 1 - Sensibilità a Fattori Esterni ed Eccezionalità:} Formato esclusivamente da \textbf{Allegiant Air (\texttt{G4})}.
    Questo vettore si distingue per un profilo unico caratterizzato da un'altissima sensibilità a fattori non direttamente controllabili: registra il valore più elevato di \textit{security delay} ($0,15$ minuti, un ordine di grandezza superiore alla media) e un significativo impatto meteorologico ($2,02$ minuti). Nonostante un ritardo di partenza elevato ($13,87$ minuti), mantiene un tasso di cancellazione tra i più bassi ($0,66\%$), indicando una politica di "volo a tutti i costi" nonostante le avversità esterne.
    \item \textbf{Cluster 2 - Alta Efficienza e Stabilità di Rete:} Include \texttt{UA}, \texttt{NK}, \texttt{DL}, \texttt{MQ}, \texttt{HA}, \texttt{YX}, \texttt{AS}, \texttt{WN}.
    È il cluster più numeroso e performante. I vettori in questo gruppo, tra cui i colossi \texttt{UA} e \texttt{DL}, mostrano ritardi all'arrivo estremamente contenuti (minimo di $0,69$ minuti per \texttt{YX}) e una grande capacità di recupero in volo (\textit{makeup} superiore ai $6$ minuti per molti membri). \texttt{HA}, precedentemente isolata per la sua natura geografica, si unisce ora a questo gruppo grazie alla sua stabilità operativa d'eccellenza.
\end{itemize}
...
In questa appendice vengono riportati i Directed Acyclic Graphs (DAG) generati da Spark per le implementazioni RDD delle prime tre query. Questi grafici illustrano le fasi di trasformazione, gli shuffle e la suddivisione in stage del piano di esecuzione distribuito.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{../charts/spark_dag/monthly_performance_rdd}
    \caption{DAG di Spark per la Query 1 (RDD): evidenzia la fase di \textit{aggregateByKey} e la successiva trasformazione finale.}
    \label{fig:dag_q1}
\end{figure}

\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=1\textwidth]{../charts/spark_dag/Query2_job0_rdd}
    \vspace{0.5cm}
    \includegraphics[width=1\textwidth]{../charts/spark_dag/Query2_job1_rdd}
    \vspace{0.5cm}
    \includegraphics[width=1\textwidth]{../charts/spark_dag/Query2_job2_rdd}
    \caption{DAG di Spark per la Query 2 (RDD): i tre job mostrano rispettivamente il filtraggio/aggregazione, il calcolo delle medie e l'ordinamento per il ranking top-10.}
    \label{fig:dag_q2}
\end{figure*}

\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=1\textwidth]{../charts/spark_dag/Query3_job0_rdd}
    \vspace{0.5cm}
    \includegraphics[width=1\textwidth]{../charts/spark_dag/Query3_job1_rdd}
    \caption{DAG di Spark per la Query 3 (RDD): i due job paralleli gestiscono rispettivamente la pipeline degli sketch per i percentili e la pipeline per il calcolo dei minimi e massimi globali.}
    \label{fig:dag_q3}
\end{figure*}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{../charts/result/Q3_median_distribution_detail_AA}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{../charts/result/Q3_median_distribution_detail_DL}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{../charts/result/Q3_median_distribution_detail_UA}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{../charts/result/Q3_median_distribution_detail_WN}
