...
La proiezione spaziale tramite \textbf{Principal Component Analysis (PCA)} (Fig.~\ref{fig:results_q4_cluster_distribution}) e la matrice di classificazione delineano quattro cluster operativi ben definiti (i valori medi delle feature per ciascun cluster sono riportati in Fig.~\ref{fig:results_q4_cluster_center} e sono stati normalizzati per evidenziare le differenze relative tra i cluster):

\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.9\textwidth]{../charts/result/Q4_cluster_center}
    \caption{Valori medi delle feature per ciascun cluster identificato nella Query 4. Le barre rappresentano le medie normalizzate delle singole features.}
...
\begin{itemize}
    ...
    Questo è il cluster più numeroso ed è caratterizzato da ritardi medi molto contenuti e tassi di cancellazione inferiori all'$1,2\%$.
...
%     \caption{Risultati Query 4: Coordinate dei centroidi per i cluster identificati.}
%     \label{fig:results_q4_cluster_center}
% \end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\textwidth]{../charts/result/Q4_clustering_table}
    \caption{Risultati Query 4: Tabella riepilogativa delle compagnie aeree e dei cluster di appartenenza.}
    \label{fig:results_q4_clustering_table}
\end{figure}
