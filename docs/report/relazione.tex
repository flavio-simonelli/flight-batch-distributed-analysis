...
Pertanto, per garantire l'integrità dei calcoli, è stata implementata una precisa logica di imputazione condizionale: se un volo presenta almeno una causa di ritardo dichiarata e non nulla, le restanti cause nulle vengono imputate a $0.0$, poiché è certo che non abbiano contribuito al disservizio complessivo.
Tuttavia, se tutte le cause di un volo risultano simultaneamente nulle (caso tipico dei ritardi lievi non classificabili), esse vengono volutamente mantenute tali.

La scelta di preservare l'assenza di dato per i voli con cause interamente omesse, anziché azzerarle massivamente, risponde a una precisa necessità di rigore statistico nel calcolo del contributo medio di ogni componente.
Se si imputassero a zero i campi di tutti i voli con ritardi inferiori ai 15 minuti, questi record verrebbero inclusi nel denominatore durante il calcolo delle medie.
Di conseguenza, l'elevato volume di questi voli con ritardi leggeri andrebbe a diluire artificialmente i risultati, distorcendo al ribasso l'impatto reale delle singole inefficienze operative.

Mantenendo il valore nullo, il motore di calcolo distribuito esclude nativamente tali record dalle operazioni di riduzione e aggregazione.
In questo modo, la determinazione del contributo medio si concentra esclusivamente sul sottoinsieme di dati in cui il ritardo è stato conclamato e categorizzato.
L'imputazione a zero viene quindi limitata alla normalizzazione interna dei voli parzialmente descritti, mentre la conservazione del nullo agisce come un filtro statistico implicito che protegge le medie dall'effetto di diluizione e permette di isolare i reali profili di criticità dei carrier.
