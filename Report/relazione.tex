...
Nelle sezioni successive, esploreremo in dettaglio come la modularità implementata a livello di codice sorgente, attraverso l'utilizzo di variabili nei vari framework e l'uso estensivo dei pattern GoF e interfacce astratte in Java per Spark, si rifletta coerentemente nella modularità dell'infrastruttura di deployment.
...
Il flusso \texttt{SAE\_Ingestion\_Pipeline} implementa un sofisticato processo di \textit{Extract-Load-Transform} articolato nelle seguenti fasi:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Partition Pruning}: Prevede la strutturazione fisica del Data Lake in directory gerarchiche basate sui valori di specifiche chiavi dove NiFi instrada dinamicamente i record Parquet in base al contenuto.
...
\begin{itemize}
    ...
    Nello specifico, se è presente almeno una causa di ritardo diversa da zero, i restanti valori nulli vengono sovrascritti a 0.
...
Questo strato funge da \textit{Single Source of Truth}, sfruttando la capacità intrinseca del Data Lake di accogliere e centralizzare dati eterogenei.
...
\begin{itemize}
    ...
    Al suo posto è stato utilizzato \texttt{aggregateByKey}, il quale combina i valori parzialmente all'interno di ogni singola partizione del nodo worker prima di trasferirli sulla rete, riducendo drasticamente il volume di dati movimentato nello shuffle.
...
In ambiente standalone su AWS EC2 (osservando la strategia Partition Pruning), l'approccio RDD fa registrare un'eccezione: una mediana del tempo di calcolo inferiore ($\sim 15s$ contro i $\sim 18s$ dei backend strutturati) e un box plot più compatto.
Questo comportamento su EC2 è riconducibile al minor overhead di astrazione dell'API RDD per computazioni non lineari: l'uso esplicito dell'operatore \texttt{combineByKey} per i buffer risulta localmente più efficiente del piano generato da Catalyst per la funzione \texttt{percentile\_approx}.

Il paradigma si ribalta radicalmente traslando l'esecuzione sull'ambiente managed AWS EMR. In questo contesto, il vantaggio dell'API RDD per la Query 3 viene completamente azzerato: DataFrame e SQL riprendono il primato prestazionale ($\sim 8.0s$ contro gli $\sim 8.3s$ di RDD in Partition Pruning) ed esibiscono una dispersione statistica minima.
