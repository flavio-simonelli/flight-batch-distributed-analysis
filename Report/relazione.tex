...
I risultati sperimentali confermano la superiorità delle API strutturate di Spark rispetto all'approccio a basso livello e dimostrano l'efficacia del formato colonnare Parquet.
In particolare, lo studio evidenzia come l'adozione strategica di tecniche di Partition Pruning e Predicate Pushdown permetta di abbattere drasticamente l'I/O overhead in ambienti distribuiti, garantendo performance ottimali sia in implementazioni on-premise su AWS EC2 che in scenari cloud-native elastici su AWS EMR.
\end{abstract}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.43\textwidth]{../graphs/BigDataStack}
...
L'orchestrazione dei flussi di lavoro è demandata a una suite di tre DAG.
...
Il flusso \texttt{SAE\_Ingestion\_Pipeline} implementa un sofisticato processo di \textit{Extract-Load-Transform} (ELT) articolato nelle seguenti fasi:
\begin{itemize}
    ...
    Un processore calcola l'hash SHA-1 confrontandolo con la firma attesa per garantire l'integrità e il \textit{non-ripudio} del dato.
    ...
    \item \textbf{Persistenza Finale e Notifica:} I file Parquet ottimizzati vengono salvati nel Data Lake.
...
\begin{itemize}
    \item \textbf{Normalizzazione e Gestione dei Ritardi:} Tramite script dedicati nel processore \texttt{UpdateRecord}, la pipeline neutralizza i ritardi negativi errati e gestisce i valori mancanti delle singole cause.
    Nello specifico, i valori vengono mantenuti a \texttt{null} se il ritardo all'arrivo è inferiore a 15 minuti; in caso contrario, se è presente almeno una causa di ritardo diversa da zero, i restanti valori nulli vengono sovrascritti a 0.
...
Questo strato funge da \textit{Single Source of Truth}, sfruttando la capacità intrinseca del Data Lake di accogliere e centralizzare dati eterogenei (strutturati, semi-strutturati e grezzi) senza la necessità di un pre-processing o di uno schema rigido preventivo.
...
Tramite la classe \texttt{FlightRepositoryFactory}, l'applicazione istanzia a runtime implementazioni specifiche basandosi su un file di configurazione YAML, gestiti attraverso la classe \texttt{ApplicationConfig}.
...
Configurato per interrogare sia CockroachDB (per i risultati di business) che Redis (per la telemetria tecnica tramit RediSearch), il sistema fornisce dashboard interattive progettate per presentare:
\begin{itemize}
...
Parallelamente, per offrire una maggiore flessibilità analitica, il sistema integra anche l'algoritmo T-Digest. Entrambi gli stimatori sono configurabili a runtime tramite la proprietà \texttt{percentileAlgorithm} in \texttt{ApplicationConfig}.
...
L'osservazione dei box plot in Appendice~\ref{sec:appendix_telemetry} rivela un comportamento duale dei backend computazionali, strettamente legato alla natura algoritmica della query eseguita.

Nello scenario di aggregazione massiva della Query 1, le API strutturate presentano mediane prestazionali generalmente inferiori rispetto all'approccio RDD, mostrando anche una dispersione statistica contratta.
...
% Catalyst esegue l'ottimizzazione logica del piano di esecuzione prima di compilare il bytecode Java, assicurando che la pipeline fisica sia lineare.
% Dal canto suo, Tungsten memorizza i record in formato binario off-heap compresso, aggirando l'allocazione di oggetti sulla JVM. Ciò riduce a livelli trascurabili l'attività del Garbage Collector, neutralizzando una delle fonti principali di variabilità temporale nei sistemi distribuiti.
% Al contrario, le esecuzioni basate su RDD per la Query 1 risentono del continuo passaggio di oggetti generici che carica pesantemente lo heap della JVM, innescando frequenti cicli di garbage collection che ampliano la dispersione del box plot.

Tuttavia, l'analisi delle metriche introduce un'interessante eccezione nelle Query 2 e 3 in ambiente standalone su AWS EC2. In questi specifici contesti, l'approccio RDD ha fatto registrare mediane del tempo di calcolo core inferiori e box plot più contratti rispetto alle controparti DataFrame e SQL. Questo comportamento è riconducibile al minor overhead di astrazione dell'API RDD quando applicata a computazioni fortemente atomiche o a pipeline non lineari.
Di contro, i backend DataFrame e SQL mostrano in questi scenari una dispersione leggermente superiore e la presenza di outlier isolati nelle esecuzioni basate su Predicate Pushdown, sintomo di una latenza variabile introdotta dall'ispezione ricorsiva dei metadati dei file.
...
Come evidenziato dai grafici posizionati in Appendice~\ref{sec:appendix_telemetry}, esiste un overhead di Wall Clock Time che si somma al tempo puro di calcolo degli executor Spark.
...
Il confronto tra i due ambienti evidenzia come la natura cloud-native ed elastica di AWS EMR riesca a contrarre sensibilmente la quota di Wall Clock Overhead rispetto al deployment standalone su AWS EC2. L'adozione di Amazon S3 e la gestione nativa delle chiamate riducono i tempi morti di negoziazione dei task e di bootstrap dell'applicazione, massimizzando il tempo in cui il cluster è effettivamente attivo nella computazione dei dati.

\subsubsection{Confronto Tecniche di Ottimizzazione}
...
L'osservazione dei tempi di esecuzione globali evidenzia come, a parità di query e di motore computazionale, le due tecniche garantiscano spesso un tempo di calcolo complessivo molto simile.
Tuttavia, l'analisi approfondita delle metriche di basso livello rivela differenze radicali nel comportamento del cluster, in particolare per quanto concerne i volumi di I/O e la pressione computazionale sui nodi worker.

Il \textbf{Partition Pruning} si dimostra l'approccio più efficiente dal punto di vista infrastrutturale.
Escludendo a monte le partizioni non necessarie a livello di file system, Spark evita del tutto l'apertura e la scansione dei file irrilevanti.
...
L'analisi della \textit{Executor Workload Skewness} dimostra infatti che, con l'utilizzo del pruning, il volume medio di dati assegnato ai nodi si allinea in modo omogeneo al picco massimo effettivamente elaborato dal singolo esecutore.
Questo parallelismo ben bilanciato garantisce l'assenza di nodi sovraccarichi.

Al contrario, il \textbf{Predicate Pushdown} richiede comunque che Spark acceda ai file Parquet per ispezionarne i metadati nei singoli footer.
Sebbene questa tecnica si confermi efficace nel ridurre i record effettivamente passati alle fasi successive della pipeline, essa genera un volume di lettura fisicamente molto più elevato sull'infrastruttura di storage e introduce un forte sbilanciamento del carico.
Come evidenziato dalla telemetria, il divario tra la media teorica e il picco massimo di byte letti risulta marcato: le dinamiche di partizionamento dei task portano tendenzialmente un singolo nodo a farsi carico della quasi totalità della scansione iniziale, trasformandolo in un temporaneo collo di bottiglia.

Questo divario nell'efficienza di lettura e nel bilanciamento dei task è particolarmente evidente nell'analisi dei percentili orari, dove il Partition Pruning riesce a contenere l'I/O overhead garantendo al contempo una sottomissione dei task lineare ed equa sui nodi, mitigando lo stress infrastrutturale sia in ambiente AWS EC2 che nella controparte elastica AWS EMR\@.
...
\begin{itemize}
    ...
    Il ritardo medio alla partenza scende da $12,8$ minuti a gennaio fino a $8,7$ minuti ad aprile, con un tasso di cancellazioni che si stabilizza virtuosamente sotto lo $0,50\%$.
...
\textbf{PSA Airlines (OH)} risulta il vettore in assoluto più inefficiente ($17,16$ minuti di ritardo medio), penalizzato in modo massiccio dal \textit{late aircraft delay}.
...
Al contrario, \textbf{United Airlines (UA)} mantiene il ritardo all'arrivo più basso tra i grandi vettori ($2,46$ minuti) e un'ottima gestione interna, ma subisce il forte impatto del \textit{NAS delay} (ritardi legati al controllo del traffico aereo), a dimostrazione di una maggiore esposizione alla congestione fisiologica degli scali.

\subsubsection{Query 3: Distribuzione Oraria dei Percentili di Ritardo}
L'analisi oraria (riportata in Appendice~\ref{sec:appendix_results}) conferma un deterioramento fisiologico delle performance con il passare delle ore, ma con impatti molto diversi tra i vettori.
Il 90° percentile di American Airlines, ad esempio, schizza oltre i $70$ minuti nella fascia serale, evidenziando una severa incapacità di smaltire la congestione diurna.
Delta Air Lines, di contro, mantiene una curva di crescita molto più dolce e controllata.

Un aspetto metodologico cruciale per SAE emerge dal confronto degli algoritmi: i grafici dimostrano che i risultati ottenuti tramite l'approssimazione KLL per il calcolo della mediana coincidono quasi perfettamente con l'algoritmo esatto GK integrato nelle API strutturate.
Questa convergenza valida l'efficacia del sistema, che riesce a garantire insight altamente precisi pur sfruttando tecniche di calcolo probabilistico per abbattere l'overhead computazionale.

\subsubsection{Query 4: Clustering delle Compagnie Aeree}
L'applicazione dell'algoritmo K-Means con selezione automatica del modello tramite Silhouette Score ha identificato $K=3$ come il numero ottimale di raggruppamenti statistici per descrivere i vettori (Fig.~\ref{fig:results_q4_cluster_distribution} in Appendice~\ref{sec:appendix_results}).
Per scopi di visualizzazione bidimensionale, è stata utilizzata una \textbf{Principal Component Analysis} per proiettare i centroidi e l'appartenenza dei vettori (Fig.~\ref{fig:results_q4_cluster_distribution}). Tuttavia, è fondamentale sottolineare che tale proiezione spaziale a due sole componenti principali offre una rappresentazione semplificata e parzialmente approssimata, comportando una fisiologica perdita di varianza.
...
\begin{itemize}
    \item \textbf{Cluster 0 - Inefficienza Operativa e Propagazione:} Comprende \texttt{AA}, \texttt{B6}, \texttt{F9}, \texttt{OH}, \texttt{OO}.
    È il raggruppamento che presenta le criticità maggiori.
    ...
    \item \textbf{Cluster 1 - Sensibilità Esterna e "Volo a tutti i costi":} Formato unicamente da \textbf{Allegiant Air (\texttt{G4})}.
    Questo vettore delinea un profilo anomalo: subisce forti ritardi in partenza a causa di fattori esterni incontrollabili (ritardi meteorologici e di sicurezza, quest'ultimo con valori un ordine di grandezza sopra la media).
    ...
    \item \textbf{Cluster 2 - Alta Efficienza e Stabilità:} Include \texttt{UA}, \texttt{NK}, \texttt{DL}, \texttt{MQ}, \texttt{HA}, \texttt{YX}, \texttt{AS}, \texttt{WN}.
    È il gruppo nettamente più virtuoso.
...
Il progetto Sky Analytics Engine dimostra che l'implementazione rigorosa di pattern architetturali distribuiti e l'adozione strategica di formati dati ottimizzati rappresentano requisiti imprescindibili per dominare la complessità inerente ai Big Data.
...
I risultati sperimentali hanno confermato in modo inequivocabile la validità delle scelte ingegneristiche effettuate.
L'utilizzo combinato del formato colonnare Parquet e delle tecniche di Partition Pruning ha consentito di abbattere drasticamente l'I/O overhead sul file system distribuito, garantendo performance ottimali.
Inoltre, l'integrazione di tecniche algoritmiche avanzate, quali gli stimatori probabilistici Quantile Sketches e il Model Selection automatizzato tramite Silhouette Score, ha dotato SAE di capacità analitiche di altissimo livello, mantenendo al contempo rigidamente confinati i costi computazionali e infrastrutturali.

Sotto il profilo del dominio applicativo, la piattaforma ha dimostrato sul campo la capacità di trasformare moli di dati grezzi in insight operativi di grande valore strategico.
Il sistema è riuscito a decodificare dinamiche complesse, come la propagazione a catena dei ritardi aeronautici, arrivando a profilare con estrema precisione l'efficienza delle singole compagnie aeree attraverso l'applicazione di modelli di machine learning non supervisionato.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{../charts/spark_dag/monthly_performance_rdd}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.255\textwidth]{../charts/spark_dag/Query2_job0_rdd}
    \includegraphics[width=0.34\textwidth]{../charts/spark_dag/Query2_job1_rdd}
    \includegraphics[width=0.35\textwidth]{../charts/spark_dag/Query2_job2_rdd}
...
\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=0.41\textwidth]{../charts/spark_dag/Query3_job0_rdd}
    \includegraphics[width=0.44\textwidth]{../charts/spark_dag/Query3_job1_rdd}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\textwidth]{../charts/telemetry/ec2/part_prun/metrics_ec2_monthly_performance_part_prun_cropped}

    \vspace{1cm}

    \includegraphics[width=1\textwidth]{../charts/telemetry/ec2/pred_push/metrics_ec2_monthly_performance_pred_push_cropped}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\textwidth]{../charts/telemetry/emr/part_prun/metrics_emr_monthly_performance_part_prun_cropped}

    \vspace{1cm}

    \includegraphics[width=1\textwidth]{../charts/telemetry/emr/pred_push/metrics_emr_monthly_performance_pred_push_cropped}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\textwidth]{../charts/telemetry/ec2/part_prun/metrics_ec2_arrival_delay_ranking_part_prun_cropped}

    \vspace{1cm}

    \includegraphics[width=1\textwidth]{../charts/telemetry/ec2/pred_push/metrics_ec2_arrival_delay_ranking_pred_push_cropped}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\textwidth]{../charts/telemetry/emr/part_prun/metrics_emr_arrival_delay_ranking_part_prun_cropped}

    \vspace{1cm}

    \includegraphics[width=1\textwidth]{../charts/telemetry/emr/pred_push/metrics_emr_arrival_delay_ranking_pred_push_cropped}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\textwidth]{../charts/telemetry/ec2/part_prun/metrics_ec2_hourly_delay_percentiles_part_prun_cropped}

    \vspace{1cm}

    \includegraphics[width=1\textwidth]{../charts/telemetry/ec2/pred_push/metrics_ec2_hourly_delay_percentiles_pred_push_cropped}
    \caption{Telemetria Query 3 su AWS EC2: Partition Pruning (sopra) vs Predicate Pushdown (sotto).}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\textwidth]{../charts/telemetry/emr/part_prun/metrics_emr_hourly_delay_percentiles_part_prun_cropped}

    \vspace{1cm}

    \includegraphics[width=1\textwidth]{../charts/telemetry/emr/pred_push/metrics_emr_hourly_delay_percentiles_pred_push_cropped}
    \caption{Telemetria Query 3 su AWS EMR: Partition Pruning (sopra) vs Predicate Pushdown (sotto).}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\textwidth]{../charts/telemetry/ec2/part_prun/metrics_ec2_airline_clustering_part_prun_cropped}

    \vspace{1cm}

    \includegraphics[width=1\textwidth]{../charts/telemetry/ec2/pred_push/metrics_ec2_airline_clustering_pred_push_cropped}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\textwidth]{../charts/telemetry/emr/part_prun/metrics_emr_airline_clustering_part_prun_cropped}

    \vspace{1cm}

    \includegraphics[width=1\textwidth]{../charts/telemetry/emr/pred_push/metrics_emr_airline_clustering_pred_push_cropped}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.70\textwidth]{../charts/result/Q4_cluster_center}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.45\textwidth]{../charts/result/Q4_cluster_distribution}
